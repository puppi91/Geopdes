% SP_BSPLINE_FLUID_MOD: Construct different pair of B-Splines spaces on the physical domain for fluid problems.
%
%   [spv, spp] = sp_bspline_fluid_mod (elem_name, knots, nsub, ...
...
function [spv, spp] = sp_bspline_fluid_mod (element_name, ...
...
% In this case the regularity is assigned first in the velocity space
    ...
    [knotsp, degp] = knt_derham (knots_h1, degree_h1, 'L2');
    % here is the modification: we remove 'integral-preserving' since in
    % Geopdes the derivative on the physical domain for  p = 1/|DF| \hat p \circ F^{-1}
    % have not been implemented yet
    %spp = sp_bspline (knotsp, degp, msh, 'integral-preserving');
    spp = sp_bspline (knotsp, degp, msh);
